      ...
      <div className="item-content">
        <div>
          ...
          {!!labels?.length && <div className="labels-wrap">
            {labels.map((item, index) => <label key={index} className="uk-label bare">{item.attributes.title}</label>)}
          </div>}
          {content && <div dangerouslySetInnerHTML={{__html: content}}/>}
        </div>
        <div className="item-content-control">
          {address && <div>
            <a href="/asd" className={`${(!big && !xsmall) ? "button small" : ''}`}>{address.city}{address.street}{address.houseNr}</a>
          </div>}
          <div>
